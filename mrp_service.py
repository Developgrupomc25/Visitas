...
class mrp_service_all(osv.osv):
	...
	_columns = {
       ...
       #'clienteEntregaCargo': fields.char('Cargo', size = 30, help = 'Cargo del cliente'),	
	    ...
	    'produccion_fecha': fields.date('Fecha de la Orden', required = False),
		...
		'produccion_cliente': fields.many2one('res.partner','Cliente',required= False, help = 'Cliente al que se le hace el servicio'),
		#'produccion_fecha_inicial':  fields.date('Fecha inicial', required = False),
		...
		'state': fields.selection([('one','Borrador'),('two','Inspeccion'),('three','Cotizacion'),('four','Autorizada'),('five','Orden de trabajo'),('six','Orden de calidad'),('final','Cancelado')], 'Estado', readonly = True),
